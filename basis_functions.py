...
class linear_basis:
    ...
    def func(self,x,a,b,el_type):
        f0 = lambda x: 0.0
        f1 = lambda x: 2.0/(b-a)*(x-a)
        f2 = lambda x: 2.0/(b-a)*(b-x)
        return np.piecewise(x,
                            [x<=a, x>=b, (x>a)*(x<=(a+b)/2.0), (x<b)*(x>=(a+b)/2.0)], 
                            [f0, f0, f1, f2])
        #if (x<=a) or (x>=b):
        #    val = 0.0
        #elif x<=(a+b)/2.0:
...
class quadratic_basis:
    ...
    # Define quadratic basis function with support [a,b]
    def func(self,x,a,b,el_type):
        if el_type == 0:
            if (x<=a) or (x>=b):
                val = 0.0
    ...
    # Define gradient of basis function
    def grad(self,x,a,b,el_type):
        if el_type == 0:
            if (x<=a) or (x>=b):
                val = 0.0
